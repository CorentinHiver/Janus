...
int SaveList(int brd, double ts, uint64_t trgid, void *generic_ev, int dtq)
{
	...
	int TrefWindow = FERS_GetParam_int(handle[brd], "TrefWindow");
	...
	if (dtq & DTQ_SPECT) {
		...
		if (of_list_a != NULL) { 
			...
			for(i=0; i<MAX_NCH; i++) {
				if ((ev->chmask >> i) & 1) {
					...
					if (tmp_enL[i] >= 0 && ((GainSelect & GAIN_SEL_LOW) || GainSelect == GAIN_SEL_AUTO)) fprintf(of_list_a, "%8d ", tmp_enL[i]);
					else fprintf(of_list_a, "       - "); 
					if (tmp_enH[i] >= 0 && ((GainSelect & GAIN_SEL_HIGH) || GainSelect == GAIN_SEL_AUTO)) fprintf(of_list_a, "%8d ", tmp_enH[i]);
					else fprintf(of_list_a, "       - "); 
					if (isTSpect) {
						if (ev->tstamp[i] > 0) {
							if (J_cfg.OutFileUnit) fprintf(of_list_a, "%8.1f ", 0.5 * ev->tstamp[i]);
							else fprintf(of_list_a, "%8d ", ev->tstamp[i]);
						} else fprintf(of_list_a, "       - ");
						if (EnableToT && (ev->ToT[i] > 0) && (ev->tstamp[i] > 0)) {  // Don't write ToT if there is no ToA
							if (J_cfg.OutFileUnit) fprintf(of_list_a, "%8.1f ", 0.5 * ev->ToT[i]);
		...
		if (of_list_c != NULL) {
			for (int j = 0; j < MAX_NCH; ++j) {
				...
				if (isTSpect) {
					if (datatype & 0x10) {
						if (J_cfg.OutFileUnit) fprintf(of_list_c, ",%f", 0.5*ev->tstamp[j]);
						else fprintf(of_list_c, ",%" PRIu32, ev->tstamp[j]);
					} else fprintf(of_list_c, ",-1");
					if (datatype & 0x20) {
						if (J_cfg.OutFileUnit) fprintf(of_list_c, ",%f", 0.5 * ev->ToT[j]);
	...
	// ----------------------------------------------------------------------------------
		...
		if (of_list_a != NULL) {
			...
			if (num_of_hits == 0) {
				if (dtq & 0x80) fprintf(of_list_a, "%3d  --          -- %16.3lf %16.3f %12" PRIu64 "\t\t%d\n", brd, ts, ev->rel_tstamp_us, trgid, num_of_hits);
				else fprintf(of_list_a, "%3d  --          -- %16.3lf %12" PRIu64 "\t\t%d\n", brd, ts, trgid, num_of_hits);
			} else {
				for (i = 0; i < num_of_hits; ++i) {
	...
	// ----------------------------------------------------------------------------------
			...
			for(i=0; i<ev->nhits; i++) {
				...
				if (ev->tstamp[i] > 0) {
					if (J_cfg.AcquisitionMode == ACQMODE_TIMING_CSTART) {
						...
						if (J_cfg.OutFileUnit) fprintf(of_list_a, "%8.1f ", (TrefWindow - 0.5 * ev->tstamp[i]));
						else fprintf(of_list_a, "%8d ", (uint32_t)(TrefWindow / (float)CLK_PERIOD_5202) - ev->tstamp[i]);
					}
				}
				else fprintf(of_list_a, "       - ");
				...
				if (evg) fprintf(of_list_a, "%16.4lf\t\t%" PRIu16, fine_tstamp, ev->nhits);  
				evg = 0;
				fprintf(of_list_a, "\n");
			}
			ascii_size = ftell(of_list_a);
		...
		if (of_list_c != NULL) {
			for (int chit = 0; chit < ev->nhits; ++chit) {
				...
				if (datatype & 0x10) {
					if (J_cfg.AcquisitionMode == ACQMODE_TIMING_CSTART) {
						if (J_cfg.OutFileUnit) fprintf(of_list_c, "%f", 0.5 * ev->tstamp[chit]);
						else fprintf(of_list_c, "%" PRIu32, ev->tstamp[chit]);
					} else if (J_cfg.AcquisitionMode == ACQMODE_TIMING_CSTOP) {
						if (J_cfg.OutFileUnit) fprintf(of_list_c, "%f", (TrefWindow - 0.5 * ev->tstamp[chit]));
						else fprintf(of_list_c, "%" PRIu32, (uint32_t)(TrefWindow / (float)CLK_PERIOD_5202) - ev->tstamp[chit]);
					}
				} else fprintf(of_list_c, "-1");
				if (datatype & 0x20) {
					if (J_cfg.OutFileUnit) fprintf(of_list_c, ",%f", 0.5 * ev->ToT[chit]);
...
// ****************************************************
int WriteTempHV(uint64_t pc_tstamp, ServEvent_t sev[FERSLIB_MAX_NBRD]) {
	...
	for (int i = 0; i < J_cfg.NumBrd; ++i) {
		...
		fprintf(of_servInfo, "\t%02d\t\t%" PRIu64 "\t\t%2.2f\t\t%2.2f\t\t%2.2f\t\t%2.2f\t\t%3.2f\t%3.2f\t%d\t\t\t0x%X\t\t\t", i, sev[i].update_time,
