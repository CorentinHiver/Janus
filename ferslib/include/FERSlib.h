...
#define FERSLIB_VERSION_MINOR			0
...
#define FERSLIB_RELEASE_DATE			"31/01/2025"
/*! @} */
...
typedef struct {
	...
	uint16_t FERSCode;		//!< FERS code, e.g. 5256
	uint8_t PCBrevision;	//!< PCB revision 
...
extern "C" {
	...
	* @brief
	*/
	CAEN_FERS_DLLAPI int FERS_SendTDLSync(int handle);
	...
	*/
	 ...
	 * @param[out] flashOrEEPROM	Pointer to a flag indicating the source (0 = flash, 1 = EEPROM)
	 * @param[in]  handle			Board handle
	 * @param[in]  npts				Number of points to read
	 * @return						0 on success, or a negative error code as defined in #FERSLIB_ErrorCodes
	 */
	CAEN_FERS_DLLAPI int FERS_ReadThrCalibFromFlash(int handle, int npts, char* date, float* ThrOffset, int* flashOrEEPROM);
	...
	*/
	 ...
	 * @ingroup calib
	*/
	CAEN_FERS_DLLAPI int FERS_WriteThrCalibToFlash(int handle, int npts, float* ThrOffset);
