...
int Configure5202(int handle, int mode) {
	...
	CitirocCfg |= FERScfg[brd]->HV_Adjust_Range << crcfg_8bit_dac_ref;	// HV adjust DAC reference: 0=2.5V, 1=4.5V                   (bit 330 of SR)
	...
	for (i = 0; i < FERSLIB_MAX_NCH_5202; i++) {
		...
		if (FERScfg[brd]->HV_Adjust_Range >= 0) {
			uint32_t HV_adj_range = (uint32_t)FERScfg[brd]->HV_Adjust_Range << 8;
			ret |= FERS_WriteRegister(handle, INDIV_ADDR(a_hv_adj, i), HV_adj_range | FERScfg[brd]->HV_IndivAdj[i]);
