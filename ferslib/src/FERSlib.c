...
int FERS_OpenDevice(char *path, int *handle) 
{
	...
	if (((strstr(path, "cnc") != NULL) && (ns == 3)) && !FERS_Offline) {  // Open Concentrator only
	 ...
	 // Find free board index 
		...
		BoardIndex = i;
		FERScfg[i] = (Config_t*)calloc(1, sizeof(Config_t));
		...
		if (FERS_IsXROC(*handle)) {
			...
			if (ret != 0) {
				if (ENABLE_FERSLIB_LOGMSG) FERS_LibMsg("[WARNING][BRD %02d] Can't read pedestal calibration\n", BoardIndex);
			}
			// Set DC offset for the MuxOut (12 bit DAC)
		...
		if (ENABLE_FERSLIB_LOGMSG) FERS_LibMsg("[INFO][BRD %02d] Connected board with index %d: Type=%s, FW_Rev=%08X\n", BoardIndex, BoardIndex, FERS_BoardInfo[BoardIndex]->ModelName, FERS_BoardInfo[BoardIndex]->FPGA_FWrev);
...
// --------------------------------------------------------------------------------------------------------- 
// Description: Send a sync to FERS modules via TDL broadcast
// Inputs:		handle = concentrator handle
// Return:		0=OK, negative number = error code
// --------------------------------------------------------------------------------------------------------- 
int FERS_SendTDLSync(int handle)
{
	return LLtdl_SendSyncBroadcast(FERS_CNCINDEX(handle));
}
...
int FERS_WriteA5256EEPROMInfo(int handle, FERS_A5256_Info_t binfo) {
	...
	memcpy(bic + 6, &binfo.FERSCode, 2);		// e.g. 5256			(2)
	...
	if (strcmp(header[0], "$$$$CAEN-Spa") == 0) { // header is found in FW_File
		...
		if (retfsf == 0) {
			...
			if (board_family[board_compatibility] == 0) { // raise an error, the firmware is not compatible with the board
				char merr[128];
				sprintf(merr, "ERROR! The firmware version is not compatible with the model of board %d\n", FERS_INDEX(handle));
				(*ptr)(merr, -1); 
				return FERSLIB_ERR_UPGRADE_ERROR;
			}
		}
