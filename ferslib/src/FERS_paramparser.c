...
static int _SetDefaultConfig(int brd) {
	...
	//FERScfg[brd]->FiberDelayAdjust[MAX_NCNC][8][16];		// Fiber length (in meters) for individual tuning of the propagation delay along the TDL daisy chains
...
int FERS_SetParam(int handle, char *param_name, char *value) {
	...
	if (streq(str, "FiberDelayAdjust")) {
		...
		if ((np == 4) && (cnc >= 0) && (cnc < FERSLIB_MAX_NCNC) && (chain >= 0) && (chain < 8) && (node >= 0) && (node < 16))
			FERScfg[brd]->FiberDelayAdjust[cnc][chain][node] = length;
	}
	...
	if (streq(str, "HV_Adjust_Range")) {
		...
		else if (streq(value, "DISABLED"))						FERScfg[brd]->HV_Adjust_Range = (uint32_t)-1;
		else ValidParameterValue = 0;
	}
...
int FERS_GetParam(int handle, char *param_name, char *value) {
	...
	strcpy(value, ""); // is value remains empty, the param name is not valid
	...
	if (streq(str, "FiberDelayAdjust"))			sprintf(value, "%f", FERScfg[brd]->FiberDelayAdjust[brd][ch][node]);  // CTIN: are link and node correct? Maybe better to make a string with all values (8x16)
...
int FERS_DumpCfgSaved5203(int handle) {
	...
	FILE* fParName = NULL;
	char ff[100];
	sprintf(ff, "FERSLIB_ParCfg_brd%02d.txt", FERS_INDEX(handle));
	fParName = fopen(ff, "w");
	if (fParName == NULL) {
		return -1;
	}
